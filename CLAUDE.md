...
`deploy-dashboard.yml` deploys `dashboard/` to GitHub Pages on pushes to `main` that touch `dashboard/**`, injecting credentials from GitHub Secrets by replacing `__MQTT_BROKER_WSS__`, `__MQTT_USER__`, `__MQTT_PASS__`, and `__CLOUD_WORKER_URL__` placeholders in `dashboard/index.html`. `MQTT_USER`/`MQTT_PASS` end up public, so they must be a **read-only** (HiveMQ "Subscribe Only") credential: the dashboard never publishes in that mode (HiveMQ would disconnect it). Write actions (thresholds, Refresh, Clear Data, MQTT history requests) require the **🔒 Admin** sign-in with a separate credential kept in the browser only — gate any new `client.publish` on `canPublish`. See `docs/persistence.md` (§3, Broker credentials).
